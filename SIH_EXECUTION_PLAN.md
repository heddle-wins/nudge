...
| Measured SIH evaluation | In progress | Frozen fixtures now run through the real Chrome extension/offscreen models and record masks, backends, cold-load, scan, and round-trip time. Accuracy, OCR-residual, resource, and p50/p95 reports remain. |
...
- [x] Add the one-way egress gate and raw-image canary tests. PRs #81–#82 make the service worker retain only its own locally minted receipt; the side-panel proposal draft is screenshot-free and rejects a forged syntactically valid screenshot containing a known fake-PII marker.
...
- [ ] Report precision, recall, false negatives, over-redaction, coverage, and OCR residual leaks per PII class. (PRs #70–#76 provide actual detector-box reports and second-pass detection evidence. Independent checks against labels, DOM fusion, final rendered-mask coverage, and full per-class reporting remain.)
...
PR #77 startup verification: a fresh Chrome fixture run completed all six initial and six residue scans with the readiness handshake enabled. It produced five initial masks and zero second-pass detections. This verifies one successful cold start, not a reliability rate. Phase 5 remains **2 of 6** checklist items complete; instrumentation and partial reports do not complete its broader accuracy, resource, latency, or E2E requirements.
...
- Merged [PR #68](https://github.com/heddle-wins/nudge/pull/68): exposes the local vision model-ready duration independently from scan duration, validates that telemetry at the extension boundary, and displays it in the protected-view receipt. This is instrumentation only, not a benchmark report.
- Merged [PR #70](https://github.com/heddle-wins/nudge/pull/70): adds the controlled `npm run fixtures:vision` Chrome runner. It loads a fixture-only extension build using CDP, captures each frozen synthetic fixture, executes the actual service-worker/offscreen YuNet + PP-OCR route, and writes ignored local mask/timing/backend evidence—never screenshots or OCR strings. It also corrects the extension CSP so bundled ONNX Runtime WASM can instantiate locally. A Chrome 151 WASM run completed all six fixtures: model-ready 595.4 ms, scans 473.5–1310 ms, but only detected one government ID and one phone mask; it missed the DOM credentials, face/payment, and Devanagari fixtures. This is a measured quality gap, not a passing privacy claim.
- Merged [PR #80](https://github.com/heddle-wins/nudge/pull/80): expands PP-OCR DB text boxes using an area/perimeter unclip approximation and makes the fixture command build its test-only extension variant itself. A controlled run increased matched detector masks from four to seven; strict labelled coverage remains below the privacy gate.
- Merged [PRs #81–#82](https://github.com/heddle-wins/nudge/pull/82): keeps protected image receipts service-worker-owned. The side panel displays the exact protected preview but sends no screenshot in its proposal message, and an executable schema test rejects a forged UI-supplied fake-PII image receipt.
- Merged [PR #83](https://github.com/heddle-wins/nudge/pull/83): adds a frozen AI-generated fictional raster portrait fixture. In the controlled local run YuNet found the face at 0.894 confidence, production rendering redacted it, and the second scan found no face residue. This is one face case, not corpus-wide success.
- Merged [PR #84](https://github.com/heddle-wins/nudge/pull/84): extends deterministic Indian PII matching to PAN values with one space between each character. The current adversarial fixture still loses its PAN before matching in OCR recognition, so this is not represented as a visual-recall improvement.
- Merged [PR #85](https://github.com/heddle-wins/nudge/pull/85): emits only detected/recognized OCR region counts from the local offscreen document and includes aggregate counts in the ignored fixture report. The seven-fixture Chrome run recorded 25 detected and 23 recognized regions; the adversarial fixture recorded four detected and three recognized, localizing its PAN miss before PII matching. No OCR strings are recorded.
- Merged [PR #86](https://github.com/heddle-wins/nudge/pull/86): records Chrome DevTools heap snapshots for the offscreen vision document after local scan and residue scan. The controlled seven-fixture WASM run observed at most 6,947,696 bytes JS heap used and 10,756,096 bytes total. These are snapshots, not a process peak; CPU and GPU utilization are explicitly unavailable rather than inferred from latency.
- Latest verification: privacy-core tests (12), extension tests (41), TypeScript checks, controlled seven-fixture Chrome runs, and the extension production build all passed locally. API tests could not be rerun in this workspace because its Python environment lacks `pytest`; the recent changes did not modify API code.

**Progress:** Phase 1 is materially started (3 of 5 checklist items checked); Phase 2 has 6 of 7 items complete (local runtime, offscreen YuNet face redaction, offscreen PP-OCR, local OCR PII rules, fusion, and exact-image residue verification). Screen-state classification remains. Phase 3 is complete (6 of 6), with direct OpenAI and configured Qwen-compatible paths that remain server-only. Phase 4 is complete (6 of 6), with a documented Chrome/Chromium-only browser boundary—not Firefox support. Phase 5 has 2 of 6 checklist items complete (corpus and labels); partial progress includes reproducible bundle bytes, actual extension-context masks/timings/backends, OCR pipeline counts, and observed heap snapshots. A controlled Chrome 151 WASM run over seven frozen fixtures achieved strict labelled recall of only 1/11, despite eight masks matching a labelled region. That is insufficient for any visual-export claim: finish independent final-mask coverage/OCR-residual checks, CPU/GPU resource measurement, full capture-to-provider latency, and E2E/demo evidence before changing the image/canvas/iframe fail-closed rule.
