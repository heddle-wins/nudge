...
The `manifest.json` boxes use each source asset's pixel coordinates. The DOM credential boxes are the validated browser `getBoundingClientRect()` field bounds, rather than a hand-estimated text baseline. A browser harness must rasterize an asset at the declared dimensions, run the local offscreen detector and renderer, then pass the returned mask rectangles to `evaluateRedaction()`.
