...
let ppOcrSessions: Promise<{ detector: Awaited<ReturnType<typeof createLocalVisionSession>>["session"]; recognizer: Awaited<ReturnType<typeof createLocalVisionSession>>["session"]; vocabulary: string[] }> | undefined;
...
function localPpOcrSessions() {
  ppOcrSessions ??= Promise.all([
    ...
    loadPpOcrVocabulary()
  ]).then(([detector, recognizer, vocabulary]) => ({ detector: detector.session, recognizer: recognizer.session, vocabulary }));
...
chrome.runtime.onMessage.addListener((message: VisionRequest, _sender, sendResponse) => {
  ...
  Promise.all([localYuNetSession(), localPpOcrSessions()])
    ...
    .then(
      (regions) => sendResponse({ ok: true, requestId: message.requestId, regions }),
